...
import { FileText, Settings, Gem, Crown } from "lucide-react";
...
export default function CreatorProfilePage() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const promptRes = await fetch("/api/prompts?limit=100");
        const promptData = await promptRes.json();
        const mine = (promptData.prompts || []).filter(
          (p) => p.creatorId === session.user.id
        );
        setPromptCount(mine.length);
      } catch (err) {
        console.error(err);
      } finally {
        setLoading(false);
  ...
  <button
    onClick={() => router.push("/dashboard/creator/update-profile")}
